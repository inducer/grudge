...
def runTestFortran(n_elem, n_in, n_out, k_inner_outer, k_inner_inner, i_inner_outer, i_inner_inner, j_inner,\
            backend="CUDA", fp_format=np.float32,nruns=10):

  kern = gen_diff_knl(n_elem,n_in,n_out,k_inner_outer,k_inner_inner,i_inner_outer,i_inner_inner, j_inner)
  ...
  if CUDA:
    ...
    kern = lp.set_options(kern, "write_code") #Outputs the code before editing it
    kern = kern.copy(target=lp.PyOpenCLTarget(my_gpu_devices[0]))
    #code = lp.generate_code_v2(kern).device_code()
    ## Print the Code
    #prog = cl.Program(ctx, code)
    #prog = prog.build()
    #ptx = prog.get_info(cl.program_info.BINARIES)[0]#.decode(errors='ignore') #Breaks pocl
  ...
  for i in range(2):
    if OPENCL:
      ...
      #evt = cl.enqueue_nd_range_kernel(queue, diff_knl, X_dev.shape, (32,32),None)
      evt, (B_dev1, B_dev2, B_dev3) = kern(queue, mat1=A_dev1, mat2=A_dev2, mat3=A_dev3, vec=X_dev)
      events.append(evt)
    elif CUDA:
        diff_fn(B_dev1, B_dev2, B_dev3, A_dev1, A_dev2, A_dev3, X_dev, block=(yblk,xblk,1), grid=(ygrid,xgrid))
  if OPENCL: 
...
def runTest(n_elem, n_in, n_out, k_inner_outer, k_inner_inner, i_inner_outer, i_inner_inner, j_inner,\
            backend="CUDA", fp_format=np.float32,nruns=10):

  kern = gen_diff_knl(n_elem,n_in,n_out,fp_format=fp_format)
  transformations = generateTransformationList(k_inner_outer,k_inner_inner,i_inner_outer,i_inner_inner, j_inner)
...
def analyzeResult(n_out, n_in, n_elem, peak_gflops, device_memory_bandwidth, avg_time, fp_bytes=4):
  ...
  ideal_total_bytes_transferred = fp_bytes*(3*(n_out * n_elem) + (n_in * n_elem) + 3*(n_out * n_in))
...
def verifyResult(B_dev1, B_dev2, B_dev3, A_dev1, A_dev2, A_dev3, X_dev):
  ...
  print("Norm1: " + str(np.linalg.norm((A_host1 @ X_host) - B_host1) / np.linalg.norm(A_host1 @ X_host) ))
  print("Norm2: " + str(np.linalg.norm((A_host2 @ X_host) - B_host2) / np.linalg.norm(A_host2 @ X_host) ))
  print("Norm3: " + str(np.linalg.norm((A_host3 @ X_host) - B_host3) / np.linalg.norm(A_host3 @ X_host) ))
...
def verifyResultFortran(B_dev1, B_dev2, B_dev3, A_dev1, A_dev2, A_dev3, X_dev):
  ...
  print("Fraction Nonzero: "+ str(np.count_nonzero(errMat)/(n_out*n_elem)))
  print("Norm1: " + str(np.linalg.norm((A_host1 @ X_host).T - B_host1) / np.linalg.norm(A_host1 @ X_host) ))
  print("Norm2: " + str(np.linalg.norm((A_host2 @ X_host).T - B_host2) / np.linalg.norm(A_host2 @ X_host) ))
  print("Norm3: " + str(np.linalg.norm((A_host3 @ X_host).T - B_host3) / np.linalg.norm(A_host3 @ X_host) ))

def k_inner_inner_options(reverse=True):
  return sorted([8,16,32], reverse=reverse)

def k_inner_outer_options(n_in, k_inner_inner, sm_size, fp_bytes=4,reverse=False):
...
def exhaustiveSearch(n_in, n_out, n_elem, sm_size,time_limit=float('inf'), fp_bytes=4, max_gflops=None, device_memory_bandwidth=None, max_workitems=1024, gflops_cutoff=.95, bandwidth_cutoff=0.95):
  ...
  for kii in k_inner_inner_opt:
    for kio in k_inner_outer_options(n_in, kii, sm_size, fp_bytes=fp_bytes):
      for iii in i_inner_inner_options(n_out, kii, max_workitems=max_workitems):
        for iio in i_inner_outer_options(n_out, iii):
          for ji in j_inner_opt:
            dev_arrays, avg_time = runTest(int(n_elem), int(n_in), int(n_out), int(kio), int(kii), int(iio), int(iii), int(ji))
            if not max_gflops == None and not device_memory_bandwidth == None:
              frac_peak_gflops, frac_peak_GBps = analyzeResult(n_out, n_in, n_elem, max_gflops, device_memory_bandwidth, avg_time)
              if frac_peak_gflops >= gflops_cutoff or frac_peak_GBps >= bandwidth_cutoff:
                # Should validate result here
                print("Performance is within tolerance of peak bandwith or flop rate. Terminating search")
...
def randomSearch(n_in, n_out, n_elem, sm_size, time_limit=float('inf'), fp_bytes=4, max_gflops=None, device_memory_bandwidth=None, max_workitems=1024, gflops_cutoff=.95, bandwidth_cutoff=0.95):
...
fp_format_dict = {np.float32: (4, "FP32"), np.float64: (8, "FP64"), np.complex128: (16, "C128")}
...
if toTest:
    ...
    #settings = exhaustiveSearch(n_in, n_out, n_elem, 4*12*1024, fp_bytes=fp_bytes, max_gflops=12288, device_memory_bandwidth=540)
    settings = randomSearch(n_in, n_out, n_elem, 4*12*1024, time_limit=180, fp_bytes=fp_bytes, max_gflops=12288, device_memory_bandwidth=540)
    #settings = noSearch(n_in, n_out, n_elem, 4*12*1024, time_limit=180, fp_bytes=fp_bytes, max_gflops=12288, device_memory_bandwidth=540)
    print("FINAL RESULTS")
    print(settings)
